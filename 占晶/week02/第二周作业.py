...
规律：x是一个5维向量，一共分为5类，如果第n个数是最大的，则第是第n类
...
class TorchModel(nn.Module):
    def __init__(self, input_size):
        ...
        #self.activation = torch.softmax  # nn.softmax() 
...
# 随机生成一个5维向量，第n个数大则返回n
def build_sample():
    x = np.random.random(5)          # 生成5维随机向量
    max_index = np.argmax(x)      # 找到最大值的索引（转为1-based）
    return x, max_index
